...
// id primary key, r's data columns, and the two timestamp columns every
// resource gets.
...
// field plus the always-present updated_at bump, keyed on id.
func updateStatement(table string, fields []rastrillo.Field) string {
	var sets []string
	...
	return fmt.Sprintf("UPDATE %s SET %s WHERE id = sqlc.arg(id);\n", table, strings.Join(sets, ", "))
}
...
func queriesSQL(r rastrillo.Resource) []byte {
	...
	blocks = append(blocks, fmt.Sprintf("-- name: Update%sBasics :exec\n%s",
		singular, updateStatement(r.Name, r.Form.Basics)))

	if len(r.Form.Advanced) > 0 {
		blocks = append(blocks, fmt.Sprintf("-- name: Update%sAdvanced :exec\n%s",
			singular, updateStatement(r.Name, r.Form.Advanced)))
	}
